...
namespace AutoForm.Generate
{
	public static class Controls
	{
		static Controls()
		{
			...
			foreach(Type control in ModelControlMap.Values)
			{
				try
				{
					System.Runtime.CompilerServices.RuntimeHelpers.RunClassConstructor(control.TypeHandle);
				}catch(Exception ex){
					exceptions.Add(ex);
				}
			}
...
" + MODEL_CONTROL_PAIRS + @"
		...
		private static Type GetSubControlType(Type propertyType, String propertyName, Type modelType)
		{
			return ModelControlMap.TryGetValue(propertyType, out Type controlType) ? 
				controlType : 
				throw new Exception($""Unable to locate control for {modelType.FullName}.{propertyName}. Make sure a control is registered for {propertyType.FullName}."");
		}
